...
class Element(object):
    ...
    @classmethod
    def create(cls, id_project, i, user):
        print(i)
        building = i['b']
        location = i['lo']
        city = i['c']
            ...
            if id_project == '1142':
                if all(item in list(building.keys()) for item in
                       ['bigg__buildingIDFromOrganization', 'bigg__buildingName']) \
                        and all(item in list(city.keys()) for item in
                                ['geo__name']):
                    building_name = f"{city.get('geo__name')}-{building.get('bigg__buildingName')}"
                    print(building_name)
            else:
                if all(item in list(building.keys()) for item in
                       ['bigg__buildingIDFromOrganization', 'bigg__buildingName']):
                    building_name = building.get('bigg__buildingName')
                    print(building_name)
        else:
            if all(item in list(building.keys()) for item in
                   ['bigg__buildingIDFromOrganization', 'bigg__buildingName']) \
