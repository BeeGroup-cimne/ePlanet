...
def get_actions(session, id_project, namespace, mandatory_year):
    ...
    WHERE eem.uri contains 'https://czech.cz#' and 
    o.uri ='https://czech.cz#ORGANIZATION-karolinka'
        and s.bigg__timeSeriesEnd > datetime("2021-01-01T00:00:00Z")
                and s.bigg__timeSeriesStart < datetime("2021-12-31T00:00:00Z")
        with b, eem.bigg__energyEfficiencyMeasureInvestment AS investment, eem.bigg__energyEfficiencyMeasureCurrencyExchangeRate AS ratio, COLLECT(eemt.rdfs__comment[0]) AS eemNames """ + """RETURN {code:b.bigg__buildingIDFromOrganization, investment:investment, eemNames: eemNames, ratio: ratio} AS result"""
