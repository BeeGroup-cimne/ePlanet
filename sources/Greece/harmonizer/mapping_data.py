...
def harmonize_ts_data(raw_df: pd.DataFrame, kwargs):
    ...
    raw_df['Current record'] = pd.to_numeric(raw_df['Current record'], errors='coerce')
    aux_df = raw_df[raw_df['Current record'].notna()].copy()  # Rows with only numeric value

    aux_df['aux_value'] = aux_df['Current record'] * aux_df['Variable'].astype(float)
